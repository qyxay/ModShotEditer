...
when 'skip_forced_move' then $skip_forced_move = val
  ...
  def save_config
    $mod_config ||= @config
    ordered = {}
    %w[skip_dialogue skip_choice skip_forced_move skip_pictures unshow_title skip_uneasy quit_all_time always_travel always_settings is_developer fly_mode live_update].each do |k|
