...
class Table
  ...
  def self._load(s)
    ...
    t.instance_variable_set(:@data, s.byteslice(20, (sizes[0]*(sizes[1]||1)*(sizes[2]||1))*4).unpack('l*'))
    t
  end
...
puts "Map004 tileset_id=#{m.instance_variable_get(:@tileset_id)}"
puts "tile id min=#{ids.min} max=#{ids.max} uniq=#{ids.uniq.size}"
puts "z0 range: #{ids.select { |v| v < 384 }.inspect[0,200]}"
puts "sample top: #{ids.uniq.sort.last(10).inspect}"
# 检查 z0/z1/z2 各自的值域
[0,1,2].each do |z|
  zids = []
  (0...d.xsize).each { |x| (0...d.ysize).each { |y| v = d[x,y,z]; zids << v if v != 0 } }
  puts "z#{z}: #{zids.min}-#{zids.max} uniq=#{zids.uniq.size} sample=#{zids.uniq.sort.first(8).inspect}"
end
